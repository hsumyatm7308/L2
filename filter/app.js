...
getsortzam1btn.addEventListener("click", sortingzam1l);

function sortingzam1l() {
    // console.log("i am working") 
    var lis = [];
    for (var i = 0; i < getli.length; i++) {
        // console.log(getli[i].textContent);
...
function filter() {
    ...
    for (var x = 0; x < getli.length; x++) {
        ...
        // console.log(getli[x].getElementsByTagName('a')[0].innerHTML.toLocaleLowerCase()); 

        var getavlaue = getli[x].getElementsByTagName('a')[0].innerText.toLowerCase();

        if (getavlaue.indexOf(inputfilter) > -1) {
